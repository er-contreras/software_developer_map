...
RSpec.describe HttpServer do
  ...
  describe '#setup_server' do
    ...
    it 'prints a listening message' do
      expect do
        ...
        .to output(/Server listening on #{host}:#{port}/)
        .to_stdout
    end
  end
  ...
  describe '#setup_server when port is in use' do
    ...
    it 'failes because of port is in use' do
      expect do
        ...
        .to output(/Error: Port #{port} already in use./)
        .to_stdout
  ...
  describe '#start' do
    let(:mock_client) do
      instance_double(
        TCPSocket,
        peeraddr: ['AF_INET', 12_345, '127.0.0.1', '127.0.0.1'],
        close: nil
      )
    end
