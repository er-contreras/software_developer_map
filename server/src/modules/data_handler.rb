module DataHandler
  ...
  def handle_post_request(client, path)
    case path
    when '/post'
      # send-response should be sending the message 'Task has been created after 
      # confirm the data has been storage correctly.
      send_response(client, 204, File.read(File.join(Dir.pwd, 'server/db/data.json' )), 'text/plain') 
    end
  end
end
